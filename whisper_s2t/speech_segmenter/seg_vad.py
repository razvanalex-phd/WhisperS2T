    ...
    def __init__(
        ...
        if self.device == "cpu":
            ...
            self.vad_model = torch.jit.load(
                os.path.join(BASE_PATH, "assets/seg_vad_model_cpu.ts")
            ).to(self.device)
        else:
            self.vad_pp = torch.jit.load(
            ...
            self.vad_model = torch.jit.load(
                os.path.join(BASE_PATH, "assets/seg_vad_model_gpu.ts")
            ).to(self.device)

        self.vad_pp = torch.jit.load(os.path.join(BASE_PATH, "assets/vad_pp.ts"))
        self.vad_model = torch.jit.load(
            os.path.join(BASE_PATH, "assets/segment_vad_model.ts")
        )
