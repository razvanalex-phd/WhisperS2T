...
class Tokenizer:
    def __init__(
        ...
        if self.multilingual:
            self.task_to_token_id: dict[str, int] | None = {
                task: self.tokenizer.token_to_id(f"<|{task}|>") for task in _TASKS
            }
            self.lang_code_to_token_id: dict[str, int] | None = {
    ...
    def sot_sequence(
        ...
        sequence: list[int] = [self.sot]

        if (
            self.multilingual
            and self.lang_code_to_token_id
            and self.task_to_token_id
            and lang
            and task
        ):
