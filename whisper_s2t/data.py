...
from whisper_s2t.audio import audio_batch_generator, load_audio, pad_or_trim
from whisper_s2t.configs import *


def stitch_speech_segments(
    start_ends: list[tuple[float, float]],
    max_len: float = 27.0,
    max_silent_region: float | None = None,
) -> list[list[tuple[float, float]]]:
    speech_duration = [end - start for start, end in start_ends]

    stitched_speech_segments: list[list[tuple[float, float]]] = []

    curr_seg: list[int] = [0]
    curr_dur: float = speech_duration[0]
    idx: int = 1
...
class BasicSegmenter:
    def __init__(self, max_seg_len: float = 29.0, sampling_rate: int = 16000):
    ...
    def __call__(
        ...
        if audio_signal is None and input_file is not None:
...
class WhisperDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        audio_files: list[str | np.ndarray],
        lang_codes: list[str],
        tasks: list[str],
        initial_prompts: list[str | None],
        tokenizer: Any,
        max_initial_prompt_len: int,
        device: str = "cuda",
        dta_padding: int = 48000,
    ...
    def _get_audio_signal_from_array(self, item: int) -> np.ndarray:
        signal = self.audio_files[item]
        assert isinstance(signal, np.ndarray)
        return signal

    def _get_audio_signal_from_file(self, item: int) -> np.ndarray:
        file_path = self.audio_files[item]
        assert isinstance(file_path, str)
        signal = load_audio(file_path, return_duration=False)
        assert isinstance(signal, np.ndarray)
        return signal
    ...
    def __getitem__(self, item: int) -> tuple[np.ndarray, list[int], list[int], int]:
        audio = self.get_audio_signal(item)
        seq_len = audio.shape[-1]

        init_prompt = self.initial_prompts[item]
        if init_prompt:
            initial_prompt = " " + init_prompt.strip()
...
class WhisperDataLoader:
        ...
        if initial_prompt_max_len:
            for _ in batch:
                prompt_batch.append(
                    ...
                    + (initial_prompt_max_len - len(_[2]))
    ...
    def get_segmented_audio_signal(
        ...
        sr: int = 16000,
    ) -> list[tuple[np.ndarray, list[int], list[int], int, dict[str, Any]]]:
        if initial_prompt:
        ...
        segmented_audio_signal: list[
            tuple[np.ndarray, list[int], list[int], int, dict[str, Any]]
        ] = []
    ...
    def get_data_loader_with_vad(
        ...
        initial_prompts: list[str | None],
        batch_size: int = 16,
    ) -> Iterator[tuple[torch.Tensor, list, torch.Tensor, list[dict[str, Any]], int]]:
        segmented_audio_signal = []
        pbar_update_len: dict[int, float] = {}
        ...
        for file_id, (audio_signal, lang, task, initial_prompt) in enumerate(
            ...
            while len(segmented_audio_signal) > batch_size:
                ...
                pbar_update = int(
                    sum([pbar_update_len[_["file_id"]] for _ in seg_metadata]) * 100
                )
