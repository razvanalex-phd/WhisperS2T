...
class RunningStatus:
    def __init__(self, status_text: str, console: Console | None = None) -> None:
    ...
    def __enter__(self) -> "RunningStatus":
...
def get_single_sentence_in_one_utterance(
    transcript: list[dict],
    end_punct_marks: list[str] = ["?", "."],
) -> list[dict]:
    if "word_timestamps" not in transcript[0]:
        print(
            f"Word Timestamp not available, one utterance can have multiple sentences."
        )
        return transcript
...
def ExportVTT(
    transcript: list[dict],
    file: str,
    single_sentence_in_one_utterance: bool = False,
    end_punct_marks: list[str] = ["?", "."],
) -> None:
    if single_sentence_in_one_utterance:
        transcript = get_single_sentence_in_one_utterance(
            transcript, end_punct_marks=end_punct_marks
        )
    ...
    with open(file, "w", encoding="utf-8") as f:
        ...
        for _utt in transcript:
            f.write(
                f"{format_timestamp(_utt['start_time'])} --> {format_timestamp(_utt['end_time'])}\n{_utt['text']}\n\n"
            )
...
def ExportSRT(
    ...
    with open(file, "w", encoding="utf-8") as f:
        for i, _utt in enumerate(transcript):
            ...
            f.write(f"{i}\n")
            f.write(
                f"{format_timestamp(_utt['start_time'], always_include_hours=True, decimal_marker=',')} --> "
            )
            f.write(
                f"{format_timestamp(_utt['end_time'], always_include_hours=True, decimal_marker=',')}\n"
            )
            f.write(f"{_utt['text']}\n\n")


def ExportJSON(transcript: list[dict], file: str) -> None:
    with open(file, "w", encoding="utf-8") as f:
...
def ExportTXT(
    ...
    with open(file, "w", encoding="utf-8") as f:
        for _utt in transcript:
            f.write(
                f"[{format_timestamp(_utt['start_time'])} --> {format_timestamp(_utt['end_time'])}]: {_utt['text']}\n\n"
            )


TranscriptExporter: dict[str, Callable[[list[dict], str], None]] = {
    "txt": ExportTXT,
    "vtt": ExportVTT,
    "srt": ExportSRT,
    "tsv": ExportTSV,
...
def write_outputs(
    ...
    format: str = "json",
    ip_files: list[str] | None = None,
    op_files: list[str] | None = None,
    save_dir: str = "./",
    **kwargs: Any,
) -> None:
