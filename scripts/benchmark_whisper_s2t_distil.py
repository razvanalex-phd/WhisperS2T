...
def run(
    ...
    if backend.lower() in ["huggingface", "hf"]:
        ...
        if flash_attention:
            ...
            results_dir = (
                f"{repo_path}/results/WhisperS2T-{backend}DistilWhisper-bs_{batch_size}"
            )
    else:
        asr_options = {}
    ...
    model = whisper_s2t.load_model(
        "distil-large-v2", backend=backend, asr_options=asr_options
    )
    ...
    data["pred_text"] = [
        " ".join([_["text"] for _ in _transcript]).strip() for _transcript in out
    ]
    ...
    if eval_multilingual:
        data = pd.read_csv(
            f"{repo_path}/data/MultiLingualLongform/manifest.tsv", sep="\t"
        )
        files = [f"{repo_path}/{fn}" for fn in data["audio_path"]]
...
if __name__ == "__main__":
    ...
    run(
        ...
        eval_mp3=eval_mp3,
        eval_multilingual=eval_multilingual,
    )
