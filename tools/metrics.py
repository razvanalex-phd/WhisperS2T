        ...
        scores.update(
            {
                ...
                f"{ngram_size}-GramInsertions": NGramInsertions(
                    references, hypotheses, ngram_size=ngram_size
                ),
            }
        )
    else:
...
def word_alignment_accuracy_single(references, hypotheses, collar=0.2):
    ...
    for r_idx, h_idx in word_idx_match.items():
        if (hypotheses[h_idx]["start"] < references[r_idx]["end"]) and (
            hypotheses[h_idx]["end"] > references[r_idx]["start"]
        ):
            overlapped_words += 1

        if (hypotheses[h_idx]["start"] >= references[r_idx]["start"] - collar) and (
            hypotheses[h_idx]["end"] <= references[r_idx]["end"] + collar
        ):
